...
require("lazy").setup({
 ...
 ░░░░░    ░░░░░  ░░░░░░   ░░░░░░       ░░░      ░░░░░ ░░░░░ ░░░ ░░░░░  
    ...
    {"windwp/nvim-autopairs", event = "InsertEnter", enabled = false}, -- autopairs
    {
...
-------------------------------------------------------------------------------
-- VIMTEX-SPECIFIC MAPPINGS  (<<< START of the new code) ----------------------
-------------------------------------------------------------------------------
-- These mappings exist only while you are in a TeX/LaTeX buffer
vim.api.nvim_create_autocmd("FileType", {
  pattern = { "tex", "plaintex", "latex" },
  callback = function()
    local opts = { buffer = true, silent = true }
    -- ,c  →  compile the document
    vim.keymap.set("n", "<leader>c", "<cmd>VimtexCompile<CR>", opts)
    -- ,z  →  open/refresh the PDF viewer
    vim.keymap.set("n", "<leader>z", "<cmd>VimtexView<CR>",    opts)
  end,
})
-------------------------------------------------------------------------------
-- VIMTEX-SPECIFIC MAPPINGS  (<<< END of the new code) ------------------------
-------------------------------------------------------------------------------
...
-- 1.  Replace the whole MUtils.enter() definition with ↓↓↓
MUtils.enter = function()
  -- If the coc-pum popup is visible -> confirm the item
  if vim.fn["coc#pum#visible"]() ~= 0 then
    return vim.fn["coc#_select_confirm"]()
  end

  -- Otherwise: undo-break + real newline, then let Coc do its own on-enter hook
  local cr = vim.api.nvim_replace_termcodes("<C-g>u<CR>", true, true, true)
  return cr .. vim.fn["coc#on_enter"]()
end
...
-- Accept the current Copilot ghost-text with <C-l>
vim.keymap.set("i", "<C-l>", function()
  local ok, suggestion = pcall(require, "copilot.suggestion")
  if ok and suggestion.is_visible() then
    suggestion.accept()
  else   -- fallback: behave like a normal <C-l>
